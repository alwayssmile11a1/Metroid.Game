...
void CGame::_InitDirectX()
{
	...
	if (_d3ddv == NULL)
	{ 
		MessageBox(_hWnd, L"Error initializing Direct3D Device", L"Error", MB_OK);
		return;
	}
...
void CGame::_RenderFrame()
{
	...
	{
		RECT rect;
		//set the sprite'srect for drawing
		rect.left = bird.x;
		rect.top = bird.y;
		rect.right = bird.x + bird.width;
		rect.bottom = bird.y + bird.height;
